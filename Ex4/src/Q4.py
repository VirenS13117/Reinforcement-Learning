...
import math
from Ex4.src.plot_results import plot_performance
...
def monte_carlo_policy_control(env, num_trials, num_episodes, T, epsilon=0.1, gamma=0.99):
    ...
    for n in trange(num_trials,desc="trial",leave=False):
        ...
        q_policy = defaultdict(lambda: np.array([(1.0 / float(number_of_actions)) for k in np.arange(number_of_actions)]))
        for e in trange(num_episodes):
            ...
            G_trial.append(G)
        G_full.append(G_trial)
    return np.array(G_full)
...
if __name__ == "__main__":
    ...
    G_eps = []
    for e in epsilons:
        G_eps.append(monte_carlo_policy_control(env, num_trials=10, num_episodes=2000, T=459, epsilon=e))
    plot_performance(epsilons, ["green", "blue", "orange"], np.array(G_eps))
