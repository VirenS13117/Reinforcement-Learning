...
class StochasticGrid:
    def __init__(self):
        ...
        self.start = (0, 3)
        self.goal_state = (7, 3)
        self.actions = ["left", "right", "up", "down", "north_east", "south_east", "south_west", "north_west"]
    ...
    def make_move(self, action):
        ...
        dx, dy = 0, 0
        wind_shift = random.choice([self.wind_strength[state[0]]-1, self.wind_strength[state[0]], self.wind_strength[state[0]]+1])
        new_state = state
        if action == "up":
            dy += 1
        elif action == "down":
