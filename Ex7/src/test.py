...
class ValueFunction:
    ...
    def learn(self, position, velocity, action, target):
        ...
        active_tiles = self.get_active_tiles(position, velocity, action)
        self.weights[active_tiles] = self.weights[active_tiles] + self.step_size * (
                    target - self.value(position, velocity, action)) * 1
        ##########################################################################
    ...
    while True:
        ...
        if update_time >= 0:
            ...
            for i in range(update_time + 1, min(update_time + n, T) + 1):
                ...
                ##########################################################################
            if update_time + n < T:
                G += (gamma ** n) * value_function.value(positions[update_time + n], velocities[update_time + n],
